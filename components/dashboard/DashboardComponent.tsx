...
export default function DashboardPageComponent() {
    ...
    const { data: count, isLoading: isCountProjectsLoading } = useCountProjectsQuery();

    const { data: countSubWorkspaces, isLoading: isCountSubWorkspaceLoading } = useCountSubWorkspacesQuery();

    const { data: countUsers, isLoading: isCountUserLoading } = useCountUserQuery()
    ...
    const successRate = totalSuccess + totalFail > 0 ? (totalSuccess / (totalSuccess + totalFail) * 100).toFixed(2) : '...';

    console.log(buildHistory);

    console.log(totalSuccess, totalFail);

    const isLoading = isCountProjectsLoading || isCountSubWorkspaceLoading || isCountUserLoading;
        ...
        <div className="flex-1 space-y-6 p-8">
                ...
                <Card>
                    <CardContent className="p-6">
                        ...
                        <div className="flex items-center justify-between">
                            <div>
                                <motion.div
                                    className={`text-2xl font-bold ${Number(successRate) > 50 ? 'text-green-500' : 'text-red-500'}`}
                    ...
                    <CardContent className="p-6">
                        <div className="flex items-center justify-between space-y-0 pb-2">
                            ...
                            <div className="relative inline-block">
                                <div className="animate-pulse rounded-full bg-green-500 h-4 w-4"></div>
                                <Activity className="absolute top-0 left-0 h-4 w-4 text-green-500" />
                            </div>
                        </div>
                        <div className="flex items-center justify-between">
                            <div>
                                <div className="text-2xl text-green-500 font-bold">+ {count ? count : 0}</div>
                            </div>
                        </div>
                    </CardContent>
                </Card>
            ...
            <div className="grid gap-4 md:grid-cols-7">
                ...
                <div className="col-span-3 grid gap-4">
                    <Card>
                        <CardContent className="p-6">
                            <h3 className="font-semibold mb-4 text-purple-500 font-bold ">Recent Deployment</h3>
                            <DeploymentHistory buildHistory={buildHistory} isLoading={false} />
                        </CardContent>
                    </Card>
                    <Card>
                        <CardContent className="p-6">
                            <h3 className="font-semibold mb-4 text-purple-500 font-bold">Project Overall</h3>
                            <ProjectOverview />
                        </CardContent>
                    </Card>
                </div>
